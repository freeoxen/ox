//! Command data shapes, trait, and registry.
//!
...
/// Metadata stored at broker paths for a registered command. The
/// authoring view of a command — display label and scope — without the
/// Rust trait object.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
...
/// `RenderCtx::data`; commands remain pure with respect to observable
/// application state — the mutation is internal to the Reader
/// (lazy-decode caches in `LiveReader`/`LocalConfig`).
...
pub struct CommandCtx<'a> {
    pub registry: &'a crate::render::RendererRegistry,
...
mod tests {
    ...
    #[test]
    fn command_metadata_roundtrip() {
        json_roundtrip(CommandMetadata {
    ...
    // ----- CommandRegistry / Command trait tests -----

    use crate::key::{KeyCodeRepr, KeyModifierSet};
    use crate::render::RendererRegistry;
    use structfs_core_store::{Error, Record, Value};

    /// Minimal in-process Reader that always returns no record. Used by
    /// trait tests so we don't pull in heavier crates from horns-core.
    struct EmptyReader;

    impl Reader for EmptyReader {
        fn read(&mut self, _path: &Path) -> Result<Option<Record>, Error> {
            Ok(None)
        }
    }
