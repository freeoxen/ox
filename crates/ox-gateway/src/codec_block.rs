...
/// `true` when routes should run codec ops through the Block instead of
/// calling the native functions in-process. Same code either way — the
/// flag switches the execution substrate, not the behavior (the parity
/// suite holds the two byte-identical).
pub fn enabled() -> bool {
    matches!(
        std::env::var("OX_GATEWAY_WASM_CODEC").as_deref(),
        Ok(v) if !v.is_empty() && v != "0" && !v.eq_ignore_ascii_case("false")
    )
}

/// Route-facing decode through the Block. Runs on the blocking pool —
/// wasmtime execution must not park an async worker thread.
pub async fn decode_request(
    dialect: &'static str,
    body: serde_json::Value,
) -> Result<ox_kernel::CompletionRequest, String> {
    let result = tokio::task::spawn_blocking(move || {
        run_job(serde_json::json!({
            "op": "decode_request",
            "dialect": dialect,
            "body": body,
        }))
    ...
    .await
    .map_err(|e| format!("codec block task: {e}"))??;
    serde_json::from_value(result).map_err(|e| format!("block returned bad request: {e}"))
}

/// Route-facing buffered-response encode through the Block.
pub async fn encode_response(
    dialect: &'static str,
    events: &[ox_types::StreamEvent],
    meta: &ox_codec::ResponseMeta,
) -> Result<serde_json::Value, String> {
    let events = serde_json::to_value(events).map_err(|e| e.to_string())?;
    let meta = serde_json::json!({ "id": meta.id, "model": meta.model, "created": meta.created });
